...
app.get("/directors/",async (request,response)=>{
    ...
    FROM 
    director`;
    const director_list=await database.all(DbQuery);
    response.send(director_list.map((item)=>
    director_snake_camel(item)));
...
app.get("/directors/:directorId/movies/",async(request,reponse)=>{
    ...
    response.send(single_director.map((eachmovie)=>({movieName:eachmovie.movie_name}))
    );
});
module.exports=app;
